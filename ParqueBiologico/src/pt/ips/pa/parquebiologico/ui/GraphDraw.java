 ...
 * Object-types for vertices and edges must implement the DrawableGraphElement
 * interface.
 *
 * Text of vertices and edges is obtained through the toString() method of the
 * stored elements.
 *
 * Used references: http://fxexperience.com/2014/05/resizable-grid-using-canvas/
...
public class GraphDraw<V extends DrawableGraphElement, E extends DrawableGraphElement> extends Pane {
    ...
    private void generatePositions() {
        ...
        Collections.sort(listVertices, new Comparator<Vertex<V>>() {
            @Override
            public int compare(Vertex<V> v1, Vertex<V> v2) {
                if (v1.element().getId() > v2.element().getId()) {
                    return 1;
                } else if (v1.element().getId() < v2.element().getId()) {
                    return -1;
                }
                return 0;
            }
        ...
        for (Vertex<V> v : listVertices) {
            ...
            locationIndices.put(v.element().toString(), index);
    ...
    private boolean sideByside(VertexLocation vla, VertexLocation vlb) {
        int indexA = locationIndices.get(vla.vertex.element().toString());
        int indexB = locationIndices.get(vlb.vertex.element().toString());
     ...
     * automatically whenever any update is needed. Can be called externally to
     * forced updates, e.g., when selected edges change.
     */
    public void redraw() {
        ...
        for (Edge<E, V> edge : edges) {
            ...
            VertexLocation vla = locations.get(locationIndices.get(a.element().toString()));
            VertexLocation vlb = locations.get(locationIndices.get(b.element().toString()));
